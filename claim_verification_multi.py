...
def preprocess(data):
    ...
    tok_ip = np.zeros((len(data), 256), dtype="int32")
    sent_ip = np.zeros((len(data), 256), dtype="int32")
    pos_ip = np.zeros((len(data), 256), dtype="int32")
    masks = np.zeros((len(data), 256), dtype="int32")
...
if not os.path.exists("train_claim/train-tok.npy"):
    data, labels, ids, predicted_evidence = load_data("train_sent_new.txt")
